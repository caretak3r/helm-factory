...
`site/` is a Docusaurus (classic preset, JavaScript) site, independent of the Helm
chart itself — it documents `platform-library`, not part of the chart's render path.

- **Build**: `npm --prefix site ci && npm --prefix site run build` — must succeed
  before merging any `site/` change.
- **Sidebar IA**: driven by folder structure under `site/docs/` (autogenerated
  sidebar, one folder per top-level section, `_category_.json` sets label/position,
  `index.md` in each folder becomes that category's landing page). Add new sections
  as new folders, not by hand-editing `sidebars.js`.
- **Deploy**: `.github/workflows/docs.yaml` builds and deploys to GitHub Pages via
  `actions/deploy-pages` on push to `main`, path-filtered to `site/**` and the
  values/schema/capability-registry files most likely to make docs stale. It is a
  separate workflow from `ci.yaml`/`release.yaml` by design — a docs build failure
  must never block a chart release.
- **Hosting assumption (open decision):** `url`/`baseUrl` in
  `site/docusaurus.config.js` assume the default project GitHub Pages URL
  (`caretak3r.github.io/helm-factory`). If a custom domain is chosen instead, both
  values need updating. GitHub Pages itself is not yet enabled in repo settings —
  that's a separate admin action (Settings → Pages → Source: GitHub Actions).
- **Content status**: Getting Started and Migration Guide are ported from
  `README.md`/`docs/migration/v1-to-v2.md`. Values Reference, Capability Catalog,
