...
The full hook sequence on install — the script ConfigMap and the hook
ServiceAccount share a weight one below the Job's, so both exist strictly
before it runs, and the release ServiceAccount is never in the hook's
lifecycle at all:

```mermaid
sequenceDiagram
    accTitle: Pre-install hook ordering and ServiceAccount isolation
    accDescr: The script ConfigMap and hook ServiceAccount are created at the same hook weight, strictly before the hook Job. The release ServiceAccount is never created or deleted by the hook lifecycle, so running pods' tokens stay valid.
