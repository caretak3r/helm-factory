...
Refreshed: 2026-07-19 (completion of the interrupted `handover/2026-07-18` run) at HEAD `8d09841` — line anchors, fixture counts, k8s matrix, schema vendoring, release and governance facts re-verified after PRs #23–#38 and the v2.0.0 release. PR #39 (v2.1.0 prep) open at refresh time.
...
`platform-library/` — chart **name `platform`** (not `platform-library`), `type: library`, `version: 2.0.0` (tagged and published; PR #39 preps 2.1.0), `kubeVersion: ">=1.34.0-0 <1.37.0-0"`. Pure library: every template file is `_`-prefixed (`define` blocks only), so the chart renders nothing by itself and `helm lint platform-library/` passes trivially no matter how broken the generators are. **Only a consumer-chart render proves anything.**

### Rendering pipeline (verified in source)

Consumer chart's single template `templates/app.yaml` contains `{{ include "platform.render" . }}`. `platform.render` (`platform-library/templates/_app.yaml:116-120`) composes three tiers:

1. **`platform.app`** (`_app.yaml:1-105`) — tier-1 orchestrator. Walks ~23 opinionated objects in a fixed order (ConfigMap → hook-script ConfigMaps → Secret → Certificate → TLS secrets → self-signed TLS → mTLS → PVC → Workload → HPA → Service → managed headless Service for StatefulSets that need one (`platform.statefulset.needsManagedHeadless`, `_app.yaml:54-56`) → Ingress → GatewayAPI → NetworkPolicy → PDB → ServiceAccount → hook ServiceAccount → ServiceMonitor → PodMonitor → CronJob → pre/post hook Jobs). Each is gated on `.Values.<block>.enabled`; the five CRD-backed features (Certificate, mTLS/PeerAuthentication, GatewayAPI/HTTPRoute, ServiceMonitor, PodMonitor) carry a **second** gate: `platform.capabilities.gateOpen` (`_capabilities.tpl:273`), driven by the shared `gatedKinds` table (`:259`) that also powers the NOTES skipped-Kind warnings. Two cross-object `fail` guards live here too: certificate+tlsSelfSigned mutual exclusion (`_app.yaml:21`) and ingress-enabled-without-service (`_app.yaml:59`). Every object is wrapped in `platform.emit`.
2. **`platform.extraObjects`** (`_util.tpl:76-99`) — tier-2 generic renderer. `.Values.extraObjects` is a map of `Kind -> [specs]`; each spec goes through `platform.genericResource` (`_util.tpl:30-74`): negotiate apiVersion, stamp standard labels, stamp namespace unless cluster-scoped, pass every non-reserved top-level key through verbatim (reserved: name/namespace/labels/annotations/apiVersion/kind/clusterScoped/metadata). Cluster-scoped Kinds `fail` render unless `allowClusterScopedExtras: true` (`_util.tpl:83`).
3. **`platform.extraManifests`** (`_util.tpl:101-115`) — raw escape hatch. Map entries emitted via `toYaml`; **string entries run through `tpl` with the full root context** (values are code — arbitrary template execution).

### Capability gates (`_capabilities.tpl`) — the v2 heart

- **Registry** (`_capabilities.tpl:76-176`): `platform.capabilities.registry` is a *YAML document inside a define*, parsed with `fromYaml` at every call site. Maps every built-in Kind plus four CRD families (Gateway API, cert-manager, Istio, Prometheus Operator) to an **ordered** apiVersion preference list — first entry is preferred/newest GA.
...
- **Two negotiation modes — the load-bearing distinction:**
  ...
  - The selector between them is `isStable` (`:222`): Kind's group ∈ hardcoded built-in group list → OrDefault, else strict. `platform.genericResource` applies exactly this rule for extraObjects.
- **Why:** under `helm template` with no cluster, `.Capabilities.APIVersions` is a minimal static set (no CRDs, not even all built-in groups). On a real cluster it is live discovery. This is THE `template`-vs-cluster behavioral split the owner flagged. CI/fixtures bridge it by force-assuming CRD groups in values (see `tests/fixtures/full/values.yaml:83-87`) or `--api-versions` flags (always the full `group/version/Kind` form — bare `group/version` silently skips objects).
- `clusterScoped` set (`:304`) drives namespace stamping and the extras gate.
...
- All library defaults live under `exports.defaults` in `platform-library/values.yaml` (663 lines). Consumers depend with `import-values: [defaults]`, which merges defaults **into the consumer's root values scope**. Missing `import-values` = every generator sees empty values = the #1 consumer pitfall.
...
Zero-config output targets PSS `restricted`: `podSecurityContext`/`containerSecurityContext` enabled by default (runAsNonRoot, seccomp RuntimeDefault, drop ALL, readOnlyRootFilesystem, no privilege escalation — `values.yaml:494+`), applied identically to the main workload, CronJob, and hook Job pods. Dedicated ServiceAccount by default with `automountServiceAccountToken: false` on both SA and every pod spec; `enableServiceLinks: false`. Render-time fail-closed guardrails (all covered by negative tests in the lint gate):

| Guardrail | Where | Failure message key |
|---|---|---|
| Unpinned image (no tag, no digest) | `_helpers.tpl:111` | "are both empty" |
| Hook Job resolves to no pin (digest only inherited when repos match) | `_helpers.tpl:747` | "no tag and no digest" |
| mTLS enabled with empty `allowedPrincipals` (unless `allowAllPrincipals: true`) | `_mtls.yaml:8` | "mtls.allowedPrincipals is empty" |
| Cluster-scoped Kind in extraObjects without `allowClusterScopedExtras` | `_util.tpl:83` | "cluster-scoped Kind" |
| `secret.existingSecret` + inline `data`/`stringData` | `_secret.yaml:3` | "mutually exclusive" |
| Missing hook `scriptFile` | `_configmap-script.yaml:58` | "Script file not found" |
| `certificate.enabled` + `tlsSelfSigned.enabled` both true | `_app.yaml:21` | "both target the Secret" |
| `ingress.enabled` with `service.enabled: false` | `_app.yaml:59` | "would not exist" |
...
- Four fixture consumer charts under `tests/fixtures/` (`minimal`=3 objects, `full`=26, `stateful`=7, `daemon`=3), each depending on `file://../../../platform-library`. `tests/render.sh <fixture> [helm args]` wipes `charts/`+`Chart.lock`, copies the reference schema in as `values.schema.json`, runs `helm dependency update` + `helm template t <dir>`. All regenerated artifacts (`charts/`, `Chart.lock`, fixture `values.schema.json`) are **gitignored** — only Chart.yaml, values.yaml, templates/ are tracked per fixture.
- `scripts/lint-library.sh` is the single gate: helm lint → schema metaschema + per-fixture check-jsonschema → per-fixture render matrix across k8s 1.34–1.36 (`KUBE_VERSIONS` from `scripts/lib/schema-manifest.sh`) with **expected-object-count assertions** (`expected_kinds()` at `scripts/lint-library.sh:95-103`) → golden snapshot diff at canonical k8s 1.34 (`GOLDEN_KUBE_VERSION`, `lint-library.sh:56`; `normalize_render` redacts nondeterministic tlsSelfSigned cert data) → kubeconform strict, each matrix version validating its OWN render (`lint-library.sh:204`), against schemas **vendored** under `tests/schemas/` (`scripts/vendor-schemas.sh` — hermetic, no network) → negative render proving CRD Kinds drop when force-assume is nulled → image-pin, helm-side schema, and posture-guardrail negative tests. Every assertion uses the guarded `if out=$(...)` idiom — a bare `var=$(...)` under `set -e` aborts the gate silently — and new checks must be mutation-tested RED before shipping.
...
- Release (`.github/workflows/release.yaml`): on `v*.*.*` tag — refuses tag ≠ `Chart.yaml` version (`release.yaml:37-45`), requires a matching `^## [X.Y.Z]` CHANGELOG heading (`release.yaml:47-51`), reruns the full CI gate, `helm package` + `helm push` to `oci://ghcr.io/caretak3r/charts` (owner lowercased). Cosign signing is documented future work. **Executed for real:** tag `v2.0.0` ran green on 2026-07-15 (run 29384461101) and published `platform-2.0.0.tgz` to GHCR.
- Delivery governance (live since 2026-07-17): `main` is protected by the active `main-pr-only` ruleset — PR-only, squash-merge only, required check "lint, render matrix, kubeconform, goldens", deletion and force-push blocked, zero bypass actors (admins included). Direct pushes to `main` are rejected by the remote.
...
`fable5-review.md` is the security review that produced v2: v1 shipped security contexts disabled, `latest`-tag fallback, default SA with mounted token, wide-open mTLS, no CI, no goldens, docs drift. Commit `4fb9386` (PR #14) landed the entire v2 rework in one breaking change. Four beads tracked known follow-ups at discovery time (helm-factory-uaw, -t7u, -665, -866) — **all four closed 2026-07-11**: kubeconform now validates each matrix version's own render, schemas are vendored (no unpinned datreeio catalog), and the certificate/tlsSelfSigned mutual exclusion is a fail-closed guard (`_app.yaml:21`). CORE.md's "Known Issues" table lists the remaining accepted low-priority quirks with file:line.
...
2. **Naming namespaces:** all defines are `platform.*` — new helpers follow `platform.<noun>` or `platform.capabilities.<verb>`. (The former `global.*` umbrella helpers were removed 2026-07-12 as dead code, bead `helm-factory-b01`.)
...
7. **Label discipline:** every object gets `platform.labels` (nindent 4) + a manual `range` over `commonLabels` + block-specific labels. Selector labels (`platform.selectorLabels`) are the immutable subset — `name` + `instance` only — and `commonLabels` must never leak into ANY selector — the old Service-selector leak was fixed 2026-07-13 (bead `hf-7a1`, P0): `_service.yaml:57-58` and `_service-headless.yaml:27-28` now use `platform.selectorLabels` only. (CORE.md:102 still lists hf-7a1 as present — that row is stale, tracked for cleanup.)
...
12. **Hook-job image confusion:** `jobs.image` inherits the main image, but the main **digest** is inherited only when repositories match; a hook with a different repo and no explicit pin fails render. The old first-install deadlock (release SA not existing when pre-install hooks run) is fixed: hook Jobs get their own hook-created, weight-ordered ServiceAccount `<fullname>-preinstall`. The distinct name is load-bearing — a same-named hook copy of the release SA would let `before-hook-creation` delete the LIVE SA on every upgrade, invalidating running pods' tokens.
...
15. **`--set key=null` surprise:** in this repo's tests, `--set foo=null` *deletes* the key from coalesced values (used deliberately throughout the negative legs, e.g. `lint-library.sh:260,528`; explanatory comment at `:527`). A weaker model reading those legs may misread them as setting a literal null.

Secrets audit: none found. The only "secret-like" content is a dummy sha256 digest in `lint-library.sh:398` and render-time-generated throwaway TLS certs. Nothing to rotate.
...
| 2 | `tests/render.sh <fixture>` (minimal\|full\|stateful\|daemon) | A consumer render end-to-end with schema enforced; accepts extra helm args (`--kube-version 1.34`, `--api-versions g/v/Kind`, `--set k=v`) | Manifests on stdout, exit 0. Kind counts: minimal 3, full 26, stateful 7, daemon 3 (`tests/render.sh full \| grep -c '^kind:'`) |
| 3 | `scripts/lint-library.sh` | THE gate: lint, schema metaschema + fixture values, render matrix k8s 1.34–1.36 with count assertions, golden diffs, kubeconform matrix (vendored schemas), negative CRD-drop render, image-pin + helm-side schema + posture guardrail negative tests | Last line `==> PASS`, exit 0. Any `FAIL:` line = broken. |
...
- **`UPDATE_GOLDEN=1` leg** not executed (mutates committed goldens; forbidden this phase). Mechanism read from source (`lint-library.sh:230-240`) and is CI-exercised.
- **Release workflow end-to-end**: RESOLVED — tag `v2.0.0` ran `release.yaml` green on 2026-07-15 (run 29384461101) and published to `oci://ghcr.io/caretak3r/charts`. In-repo consumers still use `file://`.
- **Real-cluster behavior**: partially resolved — a live `kind` cluster install on 2026-07-14 verified pre-install hook ordering and the distinct hook SA (bead `hf-5oi`, closed with evidence). Live `.Capabilities` discovery and `lookup`-based tlsSelfSigned reuse remain source-verified only.
- **CORE.md:102 is stale**: it still lists the Service-selector leak (`hf-7a1`) as present, but the bead closed 2026-07-13 and the selector is fixed. Doc cleanup pending (bd write ops currently blocked by pending Dolt schema migrations).
...
- The former kubeconform gate gap (helm-factory-uaw) is FIXED (closed 2026-07-11): each matrix version now validates its OWN render inside the render loop (`lint-library.sh:203-215`). Skills may state full per-version matrix coverage.
