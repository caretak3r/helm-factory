...
1. **`platform.app`** (`_app.yaml:1-105`) — tier-1 orchestrator. Walks ~23 opinionated objects in a fixed order (ConfigMap → hook-script ConfigMaps → Secret → Certificate → TLS secrets → self-signed TLS → mTLS → PVC → Workload → HPA → Service → managed headless Service for StatefulSets that need one (`platform.statefulset.needsManagedHeadless`, `_app.yaml:54-56`) → Ingress → GatewayAPI → NetworkPolicy → PDB → ServiceAccount → hook ServiceAccount → ServiceMonitor → PodMonitor → CronJob → pre/post hook Jobs). Each is gated on `.Values.<block>.enabled`; the five CRD-backed features (Certificate, mTLS/PeerAuthentication, GatewayAPI/HTTPRoute, ServiceMonitor, PodMonitor) carry a **second** gate: `platform.capabilities.gateOpen` (`_capabilities.tpl:372`), driven by the shared `features` registry (`:271`) that also powers the NOTES skipped-Kind warnings and declares each feature's full Kind set plus its composition policy. Two cross-object `fail` guards live here too: certificate+tlsSelfSigned mutual exclusion (`_app.yaml:21`) and ingress-enabled-without-service (`_app.yaml:59`). Every object is wrapped in `platform.emit`.
...
7. **Label discipline:** every object gets `platform.labels` (nindent 4) + a manual `range` over `commonLabels` + block-specific labels. Selector labels (`platform.selectorLabels`) are the immutable subset — `name` + `instance` only — and `commonLabels` must never leak into ANY selector — the old Service-selector leak was fixed 2026-07-13 (bead `hf-7a1`, P0): `_service.yaml:57-58` and `_service-headless.yaml:27-28` now use `platform.selectorLabels` only.
...
- **CORE.md Known Issues table**: corrected 2026-07-20 — the fixed Service-selector leak (`hf-7a1`) moved from the live table to the "Fixed since v1" line, and the remaining `hf-d97` probe-guard anchor re-pointed to `_helpers.tpl:342-349`.
