...
2. **Gate placement.** `.enabled` and capability gates live in `_app.yaml` (the wrapper), AND generators usually repeat the `.enabled` guard defensively inside their own define (`_mtls.yaml:2`, `_secret.yaml:5`). Both layers. Gating must happen OUTSIDE any `fromYaml` round-trip — `fromYaml ""` yields `{}` which becomes a bogus empty doc (the gate has a negative test for `{}` docs).
3. **Calling conventions.** Helpers needing extra args take a list — `(list $top "Kind")` unpacked with `$top := index . 0` — or a dict for named args (`dict "root" . "job" $job`, see `_app.yaml:8,13`). Never assume `.` is root inside a multi-arg helper.
4. **Namespaces.** All defines are `platform.*`; new helpers: `platform.<noun>` or `platform.capabilities.<verb>`. (The former `global.*` umbrella helpers were removed 2026-07-12 as dead code — bead `helm-factory-b01`; do not reintroduce the namespace.)
5. **Labels.** Every object gets `{{- include "platform.labels" . | nindent 4 }}` plus a manual `range` over `.Values.commonLabels` (quoted values) plus block-specific labels. `platform.selectorLabels` (`_helpers.tpl:74`; immutability rationale in the comment block at `:62-72`) is the immutable subset — name + instance only; `commonLabels` must never leak into ANY selector. (The old Service-selector leak was a P0 bug, hf-7a1, fixed 2026-07-13: `_service.yaml:57-58` and `_service-headless.yaml:27-28` now use `platform.selectorLabels` only.)
6. **Values idioms.** Holder-dict for computed values (`dict "value" ...` + `set` — `_helpers.tpl:204,404`); build lists with `append` into a variable then a single `toYaml | nindent`; probes/securityContext blocks render with `omit ... "enabled"` (`_cronjob.yaml:56,82`); user-supplied scalars quoted (`{{ $v | quote }}`); numeric-capable fields via `printf "%v"` (image tag — `platform.image`, `_helpers.tpl:119`).
7. **Fail messages are prescriptive**: name the offending values path and state the fix (models: `_helpers.tpl:111`, `_mtls.yaml:8`, `_util.tpl:83`). Every new `fail` gets a matching negative test in `lint-library.sh` that greps its message substring — the message and the test change together.
...
REQUIRE_KUBECONFORM=1 REQUIRE_CHECK_JSONSCHEMA=1 scripts/lint-library.sh   # passes at HEAD 8d09841 (re-run 2026-07-19: ==> PASS)
```
...
- Reintroducing `commonLabels` into any selector — the Service-selector leak was a P0 bug (hf-7a1, fixed 2026-07-13), not a pattern; selectors are immutable on live objects, so a mutable label there orphans the workload.
