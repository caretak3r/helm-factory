...
3. **Wire into `_app.yaml`** in rendering-order position, wrapped in emit. Decide the gate mode: a CRD-backed Kind must ALSO get a row in the `platform.capabilities.features` registry (`_capabilities.tpl:271-289`) under its values-block key, listing EVERY Kind the generator can emit (first = the representative you gate in `_app.yaml`) and a `composition` policy (`atomic` = the whole set drops if any API is missing, `independent` = per-Kind skip; add a `requires:` entry for a Kind enabled by its own sub-block). Wrap the representative with `platform.capabilities.gateOpen` (`_capabilities.tpl:372-379` — it folds `featureEnabled` and `kindAvailable` together, and the same registry drives the NOTES skipped-Kind warning; pattern at `_app.yaml:24,36,66,86,90`). Secondary Kinds gate inside their own generator with the strict `apiVersionFor`. A built-in Kind gets a plain `.Values.<resource>.enabled` gate and uses `apiVersionForOrDefault` inside the generator. No exceptions exist in the codebase — see capability-gates. The gate's content anti-drift section fails on any partial wiring.
   ```
...
## Common mistakes
- Missing the registry entry: `apiVersionFor` returns `""`, wrapper gate is falsy, object silently never renders — looks like "my template is ignored". Same symptom if a gated Kind is missing from the `platform.capabilities.features` registry (`_capabilities.tpl:271-289`): `gateOpen` resolves its values block through that registry and never opens.
