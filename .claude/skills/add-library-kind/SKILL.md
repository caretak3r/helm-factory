...
2. **Registry**: add the Kind to `platform.capabilities.registry` (`_capabilities.tpl:76-176`) with an *ordered* apiVersion preference list, newest GA first. If the Kind is cluster-scoped, also add it to the `clusterScoped` set (`_capabilities.tpl:239-241`). Missing registry entry ⇒ `apiVersionFor` returns `""` ⇒ the object never renders, silently.
...
```
Golden regeneration (`UPDATE_GOLDEN=1 scripts/lint-library.sh`) not run in staging; mechanism verified from `scripts/lint-library.sh:185-188`. Review `git diff tests/golden/` afterward.
