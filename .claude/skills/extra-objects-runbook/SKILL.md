...
2. Write the spec under `.Values.extraObjects` — a map of `Kind -> [specs]`:
   ...
   extraObjects:
     Role:
       - name: myapp-reader        # required; fail message names the Kind if missing (_util.tpl:63)
         rules:
           - apiGroups: [""]
...
REQUIRE_KUBECONFORM=1 REQUIRE_CHECK_JSONSCHEMA=1 scripts/lint-library.sh   # if fixtures changed; passes at HEAD 4fb9386
```
