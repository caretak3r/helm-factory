...
```
`UPDATE_GOLDEN=1 scripts/lint-library.sh` regenerates `tests/golden/*.yaml` (mechanism at `scripts/lint-library.sh:136-139`; not run in staging — it mutates committed files). Always follow it with a manual `git diff tests/golden/` review and a clean strict-gate re-run.

## Quality bar
In merge-bar order: (1) the gate passes across the full k8s 1.31–1.36 matrix; (2) no hardening default or fail-closed guardrail was weakened to get there; (3) no consumer-facing values key was renamed, moved, or retyped without acknowledging it as a breaking change. A green gate obtained by bumping counts or regenerating goldens without reading the diff fails the bar.
...
- editing `normalize_render` (`scripts/lint-library.sh:54-56`) — the tlsSelfSigned redaction exists because offline renders mint a fresh cert every time
