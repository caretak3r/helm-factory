...
## First rule
The tag must equal the chart version. `.github/workflows/release.yaml:37-45` refuses `v<X.Y.Z>` tags that don't match `platform-library/Chart.yaml` `version:` — align Chart.yaml, CHANGELOG, and the tag BEFORE tagging, because a rejected tag still exists on the remote and needs cleanup.
...
4. Run the exact CI-strict gate locally — the release workflow reruns it (`release.yaml:47-58`: shellcheck, helm lint, metaschema, then the strict gate) and a tag never ships unvalidated code, but finding failure pre-tag is far cheaper.
...
7. The workflow then packages and pushes `dist/platform-X.Y.Z.tgz` to `oci://ghcr.io/<owner>/charts` (owner lowercased for GHCR, `release.yaml:70-72`) using the workflow `GITHUB_TOKEN` with `packages: write`. Cosign signing/provenance is tracked future work (`release.yaml:74-75`), not something to improvise.
...
REQUIRE_KUBECONFORM=1 REQUIRE_CHECK_JSONSCHEMA=1 scripts/lint-library.sh   # the gate the release reruns; passes at HEAD 4fb9386
...
```
Not verifiable locally: the tag→GHCR publish path runs only in Actions; confirmed by reading `release.yaml`, not by execution. Whether GHCR currently hosts a published version is unverified — in-repo consumers use `file://`.
