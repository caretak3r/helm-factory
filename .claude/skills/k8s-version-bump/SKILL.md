...
## Steps
1. The verified touch list (all anchors re-confirmed 2026-07-11 against the n-2 window):
   - `scripts/lint-library.sh:31` — `KUBE_VERSIONS=(1.34 1.35 1.36)`, the render + kubeconform matrix.
   - `scripts/lint-library.sh:32` — `GOLDEN_KUBE_VERSION` (canonical golden version, currently 1.34). Changing it regenerates every golden; it moves whenever the FLOOR moves, which under n-2 is most bumps.
   ...
   - Docs claiming the range: `README.md:12` (and the Helm version-skew section at `:18`), `docs/specs/platform-library-v2-architecture.md:17`, `CORE.md:161,175,284`, CHANGELOG entry.
...
tests/render.sh full --kube-version 1.37                       # spot-check a new version renders (expect 24 kinds)
REQUIRE_KUBECONFORM=1 REQUIRE_CHECK_JSONSCHEMA=1 scripts/lint-library.sh   # full matrix; currently 1.34-1.36
```
`UPDATE_GOLDEN=1 scripts/lint-library.sh` if negotiated versions changed (not run in staging; mechanism at `lint-library.sh:185-188`) — then review `git diff tests/golden/` hunk by hunk.
