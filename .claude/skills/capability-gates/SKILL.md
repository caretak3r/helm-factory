...
2. The moving parts, all in `_capabilities.tpl`:
   - `has` (`:27-42`): true when `$top.Capabilities.APIVersions.Has $gvk` OR the entry appears in `.Values.capabilities.apiVersions` (matching `group/version` or `group/version/Kind`).
   - `registry` (`:68-158`): Kind → ordered apiVersion preference list, newest GA first. It is a YAML document inside a define, parsed with `fromYaml` at every call site — one malformed line breaks every render.
   - `apiVersionFor` (`:165-173`) strict; `apiVersionForOrDefault` (`:184-195`) with fallback.
   - `isStable` (`:204-215`): Kind's first-preference group ∈ the hardcoded built-in group list → "true". This is the selector `platform.genericResource` (`_util.tpl:52-56`) uses to pick the mode for extraObjects automatically.
...
4. When changing preference order (e.g. an apiVersion went GA): remember goldens snapshot the negotiated version at k8s 1.31 — an order change is a golden change; review the diff.
