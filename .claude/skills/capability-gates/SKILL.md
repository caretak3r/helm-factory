...
2. The moving parts, all in `_capabilities.tpl`:
   ...
   - `features` (`:271-289`): the one registry of gated CRD features, keyed by values block, each row declaring the FULL Kind set its generator can emit (first = the representative gated in `_app.yaml`), a `composition` policy (`atomic` = the set renders only when EVERY Kind's API is available — mtls, because a PeerAuthentication without its AuthorizationPolicy drops the principal restriction; `independent` = per-Kind skip — gatewayApi), and an optional `requires` override for a Kind enabled by its own sub-block (`GRPCRoute` → `gatewayApi.grpcRoute`). Its literal-YAML shape is load-bearing: the lint gate parses it with sed/grep.
   - `kindRequires` (`:297`) / `featureEnabled` (`:314`) / `kindAvailable` (`:344`): the three halves `gateOpen` (`:372-379`) folds into one check — which values path must be enabled, whether it is, and whether the composition policy's APIs are all present. `skippedKinds` (`:395+`) is `gateOpen`'s exact per-Kind complement, so an atomic Kind held back by a missing partner API shows up in the NOTES warning instead of vanishing. A gated Kind missing from `features` never renders and never warns.
