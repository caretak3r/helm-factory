...
3. Per-fixture render matrix across `KUBE_VERSIONS=(1.34..1.36)` (`:31`) with expected-object-count assertions — `expected_kinds()` at `:41-49`: minimal 3, full 24, stateful 6, daemon 3. See [[golden-count-oracle]].
4. Golden snapshot diff at canonical k8s 1.34 (`GOLDEN_KUBE_VERSION`, `:32`); `normalize_render` (`:54-56`) redacts nondeterministic tlsSelfSigned cert data. `UPDATE_GOLDEN=1` regenerates (`:136-139`).
