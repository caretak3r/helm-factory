...
The selector is `isStable` (`:222-233`): Kind's first-preference group ∈ hardcoded built-in group list → OrDefault, else strict. `platform.genericResource` applies exactly this rule automatically for extraObjects (`_util.tpl:36-41`). In tier-1, the strict mode is applied through `platform.capabilities.gateOpen` (`_capabilities.tpl:273`), which folds the `.enabled` check and `apiVersionFor` into one emitter gate.
...
One sanctioned nuance: inside a CRD generator *body*, OrDefault is acceptable because the `_app.yaml` wrapper's strict gate already proved availability (`_mtls.yaml:11`, wrapper gate `_app.yaml:36`).
...
Sources: raw/capabilities-design-header.md; `_capabilities.tpl`, `_util.tpl`, `_app.yaml` read 2026-07-10, HEAD 4fb9386; anchors re-verified 2026-07-19, HEAD 8d09841.
