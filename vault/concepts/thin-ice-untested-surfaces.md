...
Other tracked-but-accepted quirks live in CORE.md's Known Issues table (raw/core-known-issues.md). Note the Service-selector `commonLabels` leak listed there was FIXED (bead hf-7a1, P0, closed 2026-07-13): `_service.yaml:57-58` and `_service-headless.yaml:27-28` now use `platform.selectorLabels` only — CORE.md's table row is stale on this point.
