...
Verified guardrail table (anchors + gate legs confirmed 2026-07-10 at HEAD 4fb9386, re-verified 2026-07-19 at HEAD 8d09841):

| Guardrail | Source | Gate leg |
|---|---|---|
| Unpinned image (no tag, no digest) | `_helpers.tpl:111` | `lint-library.sh:389+` ("image pin enforcement") |
| Hook Job resolves to no pin (digest inherited only when repos match) | `_helpers.tpl:747` | `:406-412` |
| mTLS enabled + empty `allowedPrincipals` (opt-out: `allowAllPrincipals: true`) | `_mtls.yaml:8` | `:527-541` |
| Cluster-scoped extraObjects (opt-out: `allowClusterScopedExtras: true`) | `_util.tpl:83` | `:544-550` |
| `existingSecret` + inline `data`/`stringData` | `_secret.yaml:3` | `:553-572` |
| Missing hook `scriptFile` | `_configmap-script.yaml:58` | — |
| `certificate` + `tlsSelfSigned` both enabled (added post-discovery) | `_app.yaml:21` | `:941+` ("TLS mechanism collision") |
| `ingress.enabled` without `service.enabled` (added post-discovery) | `_app.yaml:59` | `:1023+` (cross-field guards) |
| `tag: latest`, lowercase workload type | values.schema (helm-side) | `:457-523` |
